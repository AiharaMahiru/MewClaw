...
describe("readSessionOverview", () => {
  it("从持久化事件折叠 TODO、运行次数与 token 用量", async () => {
    ...
    const persistence = {
      list: vi.fn(async () => [{ header: { id } }]),
      open: vi.fn(async () => ({
        header: { id }, close: vi.fn(async () => undefined), read: async () => ({
        events: [
  ...
  it("不同完整 Scope 派生不同 session，不读取其他用户日志", async () => {
    ...
    const persistence = {
      list: vi.fn(async () => [{ header: { id: sessionIdForScope(scope, 0) } }]),
      open: vi.fn(),
    };
  ...
  it("已选择 shared session 时读取授权后的 Web 会话投影", async () => {
    ...
    const persistence = {
      list: vi.fn(async () => [{ header: { id: sharedId } }]),
      open: vi.fn(async () => ({
        header: { id: sharedId }, close: vi.fn(async () => undefined), read: async () => ({
    ...
    expect(persistence.open).toHaveBeenCalledWith(sharedId, "read");
  });
});
