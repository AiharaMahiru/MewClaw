...
/**
 * 统一 URL 策略。调用方应在保存与每次真实出站前各调用一次；后者重新 DNS
 * 解析，可防止存储后发生的 DNS rebinding 把连接导向私网。
 */
export class UrlPolicy {
  ...
  async resolveAllowed(input: string, topLevel: boolean): Promise<ResolvedUrl> {
    ...
    if (!hostname || METADATA_HOSTS.has(hostname) || hostname.endsWith(".metadata.google.internal")) throw new UrlPolicyError();
