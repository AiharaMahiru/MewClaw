...
export interface BridgeCommand {
  action: 'status' | 'bind' | 'poll' | 'result' | 'unbind' | 'sync';
  operation?: unknown;
  sessionId: string;
  generation?: string;
  revision?: string;
...
export function parseCommand(input: unknown): BridgeCommand {
  ...
  if (Object.keys(value).some(key => !['action', 'sessionId', 'generation', 'revision', 'result', 'operation'].includes(key))) throw new Error('INVALID_WORKSPACE_COMMAND');
  if (!['status', 'bind', 'poll', 'result', 'unbind', 'sync'].includes(String(value.action)) || typeof value.sessionId !== 'string' || !value.sessionId || value.sessionId.length > 512) throw new Error('INVALID_WORKSPACE_COMMAND');
  if (value.action !== 'sync' && value.operation !== undefined) throw new Error('INVALID_WORKSPACE_COMMAND');
