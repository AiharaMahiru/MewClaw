...
function scanEvents(state, session) {
  // Session intentionally exposes an immutable snapshot API instead of its
  // internal event array. Reading from the durable offset keeps live use and
  // cold reconstruction on the same official contract.
  const events = session.snapshotEvents(state.next)
  for (const event of events) {
    state.next = event.seq + 1
