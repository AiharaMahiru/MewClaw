...
**2026-09-20 续七——R68/R69/R70 三连修已上线**：① R68：`worker.production.yml` 对 `sandbox-oci` 的 patch 以 `config:` 整对象替换（Cordis `applyEntryPatches` 顶层键替换非深合并），静默抹掉 oci.overlay 的 `extraMounts`/`resources` → run_code bootstrap 挂载丢失；production overlay 补齐完整 config，组合回归测试按真实 patch 链钉住键集不变量。② R69：`podman exec` 客户端退出后 conmon 续持 fd7 → 容器进程等 hostClosed、`'close'` 等 stdio[7] EOF、`handle.done` 悬挂的三端互等死锁；`exit`/`close`/`terminate`/`error` 四路径统一 `settleControl` 主动断开 fd7（end+destroy 幂等），terminate 先断 fd7 再 SIGTERM；控制通道测试改真实 loopback socket 对，断言断开送达容器端 EOF。③ R70：液态玻璃主题下移动端文件预览透出下层聊天文字——liquid-glass 把 `--dsw-alias-bg-layer-1` 降为 68–78% alpha，better-sidebar 坞面板是纯 `div`（非 dialog/menu 语义）吃不到 surfaces 的 backdrop-filter，品牌层移动规则补的 `blur(20px)` 在部分 WebView 上声明支持却不真实渲染（软渲染/GPU 黑名单），桌面端坞面板本无模糊补偿；修复（`GLASS_PANEL_STYLE`）：`html[data-mew-glass]` 下 `[data-dsh-panel]`/`[data-dsh-float-window]` 抬回不透明 `var(--mew-canvas)`，锚定插件自有语义属性不随 CSS Modules 哈希漂移，全视口生效。验证：无规则时面板 `rgba(35,35,39,0.78)`、有规则 `rgb(23,27,39)`（headless Chromium 计算样式双向实证）。打包 `R68`（`aaefe529…`）→ `R69`（`e6f4b090…`）→ `R70-glass-panel-opaque-20260920`（SHA-256 `0deb4537c105737757191090ded4ee02eafc172be6064f23bd5bb9031909bbb6`）→ 原子翻转 → 6 服务 active、站点 200、err 日志空。SPEC：sandbox-oci.md fd7 不变量、mewclaw-brand.md §6 玻璃坞面板不透明条款已同步。
