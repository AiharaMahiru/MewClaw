...
**2026-09-20 续十——R77 右侧栏标题栏让位 WCO 零高短路修复已上线**：桌面 advanced（win32）模式用户报右栏 TabBar 顶部被原生 caption 压住、且"位置兼容模式"改设置无效。无头浏览器三对照组实证根因：`computeTitleBarStrip` 把 `wco.present` 当权威——Electron `titleBarOverlay` 下 `navigator.windowControlsOverlay` 报 `visible:true` 但 `getTitlebarAreaRect().height=0` 的退化读数直接 `return 0`，短路 `dsh-desktop-titlebar-inset` 合约参数、preset、custom 全部下游分支（`lib/client.js` 同 `src/client/titlebar-strip.ts`）。实测矩阵（生产站点 + 注入假 WCO）：修复前 zero-rect 下 strip=null；修复后落 33px inset；WCO 健康（32）与缺席两对照组不变。修复（`aec3643`，pnpm patchedDependencies 扩展 `dsh-better-sidebar@0.18.0`/`0.19.1` 两份 patch）：`if (wco.present && wco.height > 0)` 才采信 WCO 几何。打包 `R77-sidebar-wco-strip-20260920`（SHA-256 `78e6052ba11a26e33ad7d6e02cef26bed290db011f3474af13e3e23bcd8fcd95`）→ 原子翻转 → 6 服务 active。注意残余面：用户侧 `titleBarScheme` 需落在 `preset`/`custom` 或壳 stamp inset 才生效——auto + WCO 退化 + 无 inset 仍是 0（upstream 刻意 opt-in）；正解在 dsh-plugin-desktop 给 advanced 模式也 stamp `dsh-desktop-titlebar-inset=32`，属桌面端发版范畴。

**2026-09-20 续十一——R78 移动端右侧栏系统性修复已上线**：用户报移动端右栏顶部按钮堆叠在左上角（与侧栏菜单键混在一起）且文件预览打不开。无头浏览器（390×844 持久模拟 + 铸会话）实证三个独立根因：① liquid-glass `SURFACE_STYLES` 的横向 `[role="tablist"]` 收编规则（`width:fit-content;align-self:flex-start`，本意是内容区玻璃 pill）误命中右侧栏抽屉内的 dockkit strip（`data-dockkit-strip`）——strip 实测 390→184px，`stripFill`/`stripChrome` 的右缘布局失效，Exit-fullscreen/Collapse 按钮落在 x=120–156 左中区；② 品牌层 `mewclaw-rail-fab`/`rail-edge`（z108/109）高于抽屉（z40）透出堆叠，edge 热区还拦截抽屉左缘触控；③ **最深根因**：`dsh-better-sidebar` 仍读 `sessions.list.getSnapshot().current`——上游 session-controller 0.1.6 已删该字段（`SessionListState` 只余 ids/byId/phase/subagentsByParent/jobsBySession），当前会话改由 `retainedBy.mainView>0` 推导（与 ui-layout/ui-workspace 同法）；死读致 `store.setSession(undefined)` → `data-dsh-panel-host` 空渲染、`openTab` 在 `targetSessionId===undefined` 静默 return（文件预览全死）、窄屏后台 subagent 激活无法 park。修复（`a1d35c8`）：liquid-glass `surfaces.ts` 横向 tablist 规则加 `:not([data-dockkit-strip])`（父子规则同排）+ `wallpaper.test.ts` 回归断言 + SPEC 条款；mewclaw-brand `MOBILE_STYLE` 在 ≤1023px 下 `body:has([data-sidebar-right-open="true"])` 隐藏 fab/edge + SPEC 条款；better-sidebar 0.18.0/0.19.1 两份 patch 新增 `currentSessionIdOf` 共享推导（legacy `current` 优先，否则 byId 里 `retainedBy.mainView>0`），0.19.1 三处（`Sidebar.tsx`/`use-host-feeds.ts`/`native/surface.ts`）+ 0.18.0 两处（`Sidebar.tsx`/`intercept.tsx`）全部改接，`SidebarSessionSummary` 补 `retainedBy`、`current` 改可选。注意 `pnpm patch` 提取的是已应用旧补丁的基线——新 hunk 用 `diff` 基线↔安装树生成，追加在 patch 文件尾部。打包 `R78-mobile-sidebar-sessionfix-20260920`（SHA-256 `7f3b2858530f935e3144c7d651cc8e99db42e6858d05aaf6361ef1a9d2251be9`）→ 原子翻转 → 6 服务 active。生产实测（移动视口）：宿主子节点 1（底部工作台正常挂载）、抽屉 390×844、strip 390px/alignSelf:auto、Exit-fullscreen x=320/Collapse x=356 贴右缘、fab/edge display:none、点 index.html 开出 editor tab 且 Preview 真实渲染 HTML（截图存 `/tmp/e2e/r78-mobile-*.png`）；桌面 1600px WCO 矩阵 33/32/33 不变（R77 修复保持）、`data-dsh-title-bar-compat` 正常打上、全程零 console 错误。测试：liquid-glass 29、mewclaw-brand 5 全过，lint 干净。

**2026-09-21 续十二——R79 系统性审计修复 + 右栏分割线对齐已上线**：① 全仓系统性审计（typecheck/lint/1671 测试 + 鉴权/沙箱/SSRF/审计面人工走查）定位 5 处缺陷并全修（`3ba0b50`）：`composeEntries` 扁平数组改双层恢复 typecheck 门禁；`clientIp` 识别可信反代头（loopback 对端才采信 nginx 覆盖式 `X-Real-IP`，不信任任意 XFF），登录限流从全站共桶（12/min 可被一端打满锁死全站）恢复按真实 IP 分桶、审计元数据记真实 IP；nginx 模板与生产 conf 加 `location ^~ /internal/ {return 404}`（`/internal/*` 仅靠 Bearer 时 loopback 语义已被反代掏空，补纵深防御，worker 直连 13080 不受影响）；desktop-inference 审计拼接全部 user 角色消息（原仅末条，直连 API 可用历史消息藏毒绕过）；`url-policy` 新增 `guardedLookup`/`createGuardedDispatcher`——undici Agent 连接期 lookup 每次建连重解析、逐地址过 `blockedAddress`（混合结果 fail-closed、兼容 `all:true` 双回调形态、失败按 ENOTFOUND 上抛不泄地址细节），desktop inference 用户模型 fetch 全程挂该 dispatcher，消除 assertAllowed↔fetch 间的 DNS-rebinding TOCTOU。**残余**：worker 侧 `web-private-model` 走 `PiAiAdapter`，其构造不透传 fetch/dispatcher（pi-ai 底层支持 `options.fetch` 但 adapter 不透），rebinding 窗口仍在，需上游 adapter 加钩子或改低层路径。② 右栏分割线错位修复（`c98d1b1`）：右坞 dockkit strip(38)+editorHeader(41) 底边落 y=79 与主区 header 底边 y=85 错 6px，`RIGHT_PANEL_STYLE` 加 `[data-sidebar-right-panel] [class*="_editorHeader"]{box-sizing:border-box;min-height:47px}` 撑高使两线共线（底部工作台无共线对象不受影响）；SPEC mewclaw-brand §6/§8 同步。验证：针对性测试 76 过（含 url-policy 32 新增 guardedLookup 双形态/混合拒绝）、typecheck exit 0、lint 0 告警；打包 `R79-audit-fixes-rightbar-align-20260921`（SHA-256 `65d3cf70528ab500f4d4cdee51e6f47a52d34b43d1ac090eb94f298cf632b825`）→ 原子翻转 → 6 服务 active、站点 200、公网 `/internal/pairing/start` 404；生产 CDP 实测 `editorHeaderBottom=85` 与主 header 底边精确共线（截图 `/tmp/e2e/r79-align.png`）。
...
**2026-09-21 续十四——better-sidebar 0.19.1 对 primitives 0.1.6 的 IconSendOutline16 兼容补丁（待发布）**：浏览器控制台 React #130（SideChatView 渲染 undefined 组件）实证根因：`dsh-better-sidebar@0.19.1` 按 `^0.1.5-rc.1` 构建引用了 `IconSendOutline16`，而部署的 `@deepseek-ai/dsh-client-ui-primitives@0.1.6-alpha.2` 已将其改名 `IconSendOutline14`（上游会话区发送键同源实证）——SideChatView 发送按钮子树整段渲染崩。修复：`pnpm patch` 流程对 0.19.1 patch 追加 hunk，`SideChatView.tsx` import/JSX 与 `lib/client.js` 产物三处 `IconSendOutline16→IconSendOutline14`（14px 即上游新规格，28px 圆形按钮内居中无视觉差）；patch-commit 顺带把此前 node_modules 手改全部收编进正式 patch 文件（`dsh-better-sidebar@0.18.0` 传递副本未被运行时加载——web-all profile 排除，未改）。验证：dsh-web-composition 27/27。其余控制台项分诊为噪声/设计内：`generation is still not ready after 3000ms` 是 dsh-client-connection 握手超 3s 的预警（硬超时才取消，重连期瞬态）；`/plugins/events` upstream timeout 是 client-hmr 开发期 SSE 通道在生产的重连噪声（8/24 起 1369 次，先于 EO）；`/open-in-app/icon/cursor` 404 是 open-in-app 按 app id 取桌面图标、web 部署无图标时回退通用字形（客户端对 404 有去重设计）；KaTeX `strict mode warn` 是模型输出把中文包进 `$...$` 的排版告警；`.js.map` 404 是生产未随包发 sourcemap。
