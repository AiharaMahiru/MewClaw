...
**续（同日复核·二）——滑条样式逐条复刻 mwseat**：首版滑条是单行紧凑条（侧标签 + 168px×20px 细轨 + 右侧值），与 Web 版视觉差距大。改造为 mwseat 同构列布局：整宽 30px 胶囊轨道（`border-radius:999px` + `interactive-bg-hover` 内阴影）、business-primary 渐变填充、4px 刻度、22px 拇指（白边+阴影+拖动 scale 1.14）、档位值居中轨道下方（名称 600 + `n/m`）；几何参数 PAD=16/HALF=11/填充外延 5px 与 `packages/lark/model-seat` 的 `EffortSlider` 逐字段对齐。可见「思考强度」侧标签移除（官方仅 aria-label），档位描述挂 `title`。
