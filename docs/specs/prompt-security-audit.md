...
AUTH_PROMPT_AUDIT_FALLBACK_MODELS 缺省未配置：逗号分隔的备用逻辑模型链，配置后主模型
调用失败即在同一 deepseek-official 路由内按序尝试链上其余模型——凭证/配额/模型禁用等
确定性 code 同样是模型维度故障，切换仍然有意义；仅调用方中止（预算耗尽）不继续。
旧单值变量 AUTH_PROMPT_AUDIT_FALLBACK_MODEL 仍兼容解析。每次切换以 `[prompt-audit]`
warn 落日志（只含已消毒的失败类别，不透传上游正文）。
AUTH_PROMPT_AUDIT_STICKY_MS 缺省 1800000（30 分钟），0 关闭：携带 sessionId 的审计调用
（session.prompt 族；桌面推理端点以 `desktop:<userId>` 为键）在 TTL 内优先使用本会话
上次成功的模型，成功即重定指，失败即剔除；TTL 到期自然回链首。粘性表上限 1024 条，
超限先清过期再逐最旧。
...
审计不再单点依赖 deepseek-v4.1-flash：AUTH_PROMPT_AUDIT_FALLBACK_MODELS 指定备用逻辑模型
链（生产 muse-spark-1.3 → glm-5.3-flash），任一模型失败（含确定性 code——其语义为模型
维度而非请求维度）且调用方未中止时按序尝试下一个；全部失败仍失败关闭。带 sessionId 的
调用在粘性窗口内优先本会话上次成功的模型。
