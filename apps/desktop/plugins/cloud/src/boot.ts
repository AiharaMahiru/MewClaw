...
export interface DesktopBootClient { revision: string; inject: string[]; workspaceRevision?: string }

/** 登录页只报告自身表单可用；会话页由原桌面客户端等待 Cordis Loader 后报告。 */
export function desktopCloudHtml(html: string, client: DesktopBootClient, parameters: string): string {
  ...
  if (start >= 0) {
    ...
    composeDesktopGraph(graph, client, parameters);
...
/** 云端首屏与 HMR 共用桌面宿主组合。 */
export function composeDesktopGraph(graph: SessionBootGraph, client: DesktopBootClient, parameters: string): SessionBootGraph {
