...
截至 **2026-09-20，生产基线前进至 `R70-glass-panel-opaque-20260920`**（SHA-256 `0deb4537…`）：修复液态玻璃主题下移动端文件预览透出下层聊天文字——liquid-glass 将 `--dsw-alias-bg-layer-1` 降为 68–78% alpha，better-sidebar 坞面板是纯 div 吃不到 surfaces 的 backdrop-filter，而部分移动端 WebView 声明支持 backdrop-filter 却不真实渲染；现 `html[data-mew-glass]` 下 `[data-dsh-panel]`/`[data-dsh-float-window]` 抬回不透明 `--mew-canvas`（全视口，桌面同缺陷一并覆盖）。本机生产入口 `/opt/dsh/current` 指向 `releases/R70-glass-panel-opaque-20260920`；Git 分支后续提交不等于生产已发布。

截至 **2026-09-20，此前基线为 `R69-oci-fd7-close-20260920`**（SHA-256 `e6f4b090…`）：R68（`aaefe529…`）修复 `worker.production.yml` 对 `sandbox-oci.config` 的整对象替换抹掉 `extraMounts`/`resources` 致 run_code bootstrap 丢失（Cordis patch 的 `config` 是顶层键替换而非深合并，组合回归测试已钉住键集不变量）；R69 修复 `podman exec` 客户端退出后 conmon 续持 fd7 导致的收尾死锁——容器进程等 hostClosed、`'close'` 等 stdio[7] EOF、`handle.done` 悬挂会话冻结，现 exit/close/terminate/error 四路径统一主动断开 fd7。生产实测：liangshen run_code 容器内 C/Python/Node/Rust 全零退出码，会话正常落定，零残留进程。

截至 **2026-09-20，此前基线为 `R67-better-sidebar-list-port-20260920`**（SHA-256 `8255af4f…`）：R66 的 turnTail 补丁只补了 `options.id`——0.1.6 起该槽位为 **list** 语义，`select` 仅对 chain 生效，`props.matched` 恒 undefined 致组件崩溃；R67 把补丁改成与官方 `DeliverablesTail` 同形态的组件内自选择（无产出渲染 null，含 0.18.0 `lastProduced` 副作用），双版本补丁重生成。R66（SHA-256 `06d1b7c8…`）另修复 R64 回归——`session/create` 以 `workspaceId` 定位时被缺省 cwd 回写触发官方互斥校验，普通用户"打开工作区"全灭；并以 `pnpm patch` 机制修复社区包 `dsh-better-sidebar`、放开品牌门禁对非 `@deepseek-ai` 包的补丁限制、打包暂存携带 `patches/`。
...
需要 Node.js 24+、pnpm 10.30.3；生产构建使用部署指定的固定 Node 版本。DSH 核心依赖当前锁定为 `0.1.6-alpha.2`，以锁文件为准。
