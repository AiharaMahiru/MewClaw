...
截至 **2026-09-20，生产基线前进至 `R69-oci-fd7-close-20260920`**（SHA-256 `e6f4b090…`）：R68（`aaefe529…`）修复 `worker.production.yml` 对 `sandbox-oci.config` 的整对象替换抹掉 `extraMounts`/`resources` 致 run_code bootstrap 丢失（Cordis patch 的 `config` 是顶层键替换而非深合并，组合回归测试已钉住键集不变量）；R69 修复 `podman exec` 客户端退出后 conmon 续持 fd7 导致的收尾死锁——容器进程等 hostClosed、`'close'` 等 stdio[7] EOF、`handle.done` 悬挂会话冻结，现 exit/close/terminate/error 四路径统一主动断开 fd7。生产实测：liangshen run_code 容器内 C/Python/Node/Rust 全零退出码，会话正常落定，零残留进程。本机生产入口 `/opt/dsh/current` 指向 `releases/R69-oci-fd7-close-20260920`；Git 分支后续提交不等于生产已发布。
